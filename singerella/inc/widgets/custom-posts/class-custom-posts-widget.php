...
if ( ! class_exists( 'Singerella_Custom_Posts_Widget' ) ) {

	class Singerella_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Custom Posts', 'singerella' );
			$this->widget_description 	= esc_html__( 'Display custom posts your site.', 'singerella' );
			$this->widget_id			= apply_filters( 'singerella_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass		= apply_filters( 'singerella_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility				= singerella_utility()->utility;
			$this->settings 			= array(
				'title' => array(
					'type'				=> 'text',
					'value'				=> esc_html__( 'Custom Posts', 'singerella' ),
					'label'				=> esc_html__( 'Title', 'singerella' ),
				),
				'terms_type' => array(
					'type'				=> 'radio',
					'value'				=> 'category_name',
					'options'			=> array(
						'category_name' => array(
							'label'		=> esc_html__( 'Category', 'singerella' ),
							'slave'		=> 'terms_type_post_category',
						),
						'tag'			=> array(
							'label'		=> esc_html__( 'Tag', 'singerella' ),
							'slave'		=> 'terms_type_post_tag',
						),
						'post_format'	=> array(
							'label'		=> esc_html__( 'Post Format', 'singerella' ),
							'slave'		=> 'terms_type_post_format',
						),
					),
					'label'				=> esc_html__( 'Choose taxonomy type', 'singerella' ),
				),
				'category_name' => array(
					'type'				=> 'select',
					'size'				=> 1,
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select category', 'singerella' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select category', 'singerella' ),
				...
				'tag' => array(
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select tags', 'singerella' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select tags', 'singerella' ),
				...
				'post_format' => array(
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select post format', 'singerella' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select post format', 'singerella' ),
				...
				'posts_per_page' => array(
					...
					'max_value'			=> 50,
					'min_value'			=> 0,
					'label'				=> esc_html__( 'Posts count ( Set 0 to show all. )', 'singerella' ),
				),
				'post_offset' => array(
					'type'				=> 'stepper',
					'value'				=> '0',
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Offset post', 'singerella' ),
				),
				'title_length' => array(
					'type'				=> 'stepper',
					'value'				=> '10',
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Title words length ( Set 0 to hide title. )', 'singerella' ),
				),
				'excerpt_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'singerella' ),
				),
				'meta_data' => array(
					'type'				=> 'checkbox',
					'value'				=> array(
					...
					'options'				=> array(
						'date'				=> esc_html__( 'Date', 'singerella' ),
						'author'			=> esc_html__( 'Author', 'singerella' ),
						'comment_count'		=> esc_html__( 'Comment count', 'singerella' ),
						'category'			=> esc_html__( 'Category', 'singerella' ),
						'post_tag'			=> esc_html__( 'Tag', 'singerella' ),
						'more_button'		=> esc_html__( 'More Button', 'singerella' ),
					),
					'label'				=> esc_html__( 'Display post meta data', 'singerella' ),
				),
				'button_text' => array(
					'type'				=> 'text',
					'value'				=> 'Read More',
					'label'				=> esc_html__( 'Post read more button label', 'singerella' ),
				),
			);
					...
					if ( $holder_view_dir ) {
						foreach ( $posts as $post ) {
							...
							$image = $this->utility->media->get_image( array(
								'size'			=> 'singerella-thumb-s',
								'mobile_size'	=> 'singerella-thumb-s',
							...
							$count = $this->utility->meta_data->get_comment_count( array(
								...
								'sufix'			=> _n_noop( '%s comment', '%s comments', 'singerella' ),
							) );
	...
	add_action( 'widgets_init', 'singerella_register_custom_posts_widget' );
	function singerella_register_custom_posts_widget() {
		register_widget( 'Singerella_Custom_Posts_Widget' );
	}
}
