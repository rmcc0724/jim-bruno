...
if ( ! class_exists( 'Singerella_About_Widget' ) ) {

	class Singerella_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'singerella' );
			$this->widget_id          = 'singerella_widget_about';
			$this->widget_name        = esc_html__( 'About Singerella', 'singerella' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Upload', 'singerella' ),
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'singerella' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'singerella' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline', 'singerella' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'singerella' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'singerella' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, singerella_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			 ...
			 * @param Singerella_About_Widget $this     Current widget instance.
			 */
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'singerella_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'singerella_register_about_widget' );
function singerella_register_about_widget() {
	register_widget( 'Singerella_About_Widget' );
}
