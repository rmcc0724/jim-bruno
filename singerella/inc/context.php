...
function singerella_utility() {
	return singerella_theme()->get_core()->modules['cherry-utility'];
}
 ...
 * @return void
 */
function singerella_header_class( $classes = array() ) {
	$classes[] = 'site-header';
	...
	echo singerella_get_container_classes( $classes, 'header' );
}
 ...
 * @return void
 */
function singerella_content_class( $classes = array() ) {
	$classes[] = 'site-content';
	echo singerella_get_container_classes( $classes, 'content' );
}
 ...
 * @return void
 */
function singerella_footer_class( $classes = array() ) {
	$classes[] = 'site-footer invert';
	$classes[] = get_theme_mod( 'footer_layout_type' );
	echo singerella_get_container_classes( $classes, 'footer' );
}
 ...
 * @return string
 */
function singerella_get_container_classes( $classes, $target = 'content' ) {
	switch ( $target ) {
		case 'header':
 ...
 * @return void
 */
function singerella_primary_content_class( $classes = array() ) {
	echo singerella_get_layout_classes( 'content', $classes );
}
 ...
 * @return void
 */
function singerella_sidebar_class( $classes = array() ) {
	echo singerella_get_layout_classes( 'sidebar', $classes );
}
 ...
 * @return string
 */
function singerella_get_layout_classes( $layout = 'content', $classes = array() ) {
	...
	$layout_classes = ! empty( singerella_theme()->layout[ $sidebar_position ][ $sidebar_width ][ $layout ] ) ? singerella_theme()->layout[ $sidebar_position ][ $sidebar_width ][ $layout ] : array();
	...
	$layout_classes = apply_filters( "singerella_{$layout}_classes", $layout_classes );
 ...
 * @return string|void
 */
function singerella_posts_list_class( $classes = array(), $echo = true ) {
	$layout_type      = get_theme_mod( 'blog_layout_type', singerella_theme()->customizer->get_default( 'blog_layout_type' ) );
	$layout_type      = ! is_search() ? $layout_type : 'default';
	$sidebar_position = get_theme_mod( 'sidebar_position', singerella_theme()->customizer->get_default( 'sidebar_position' ) );
	...
	foreach ( $sidebars as $sidebar ) {
		if ( singerella_widget_area()->is_active_sidebar( $sidebar ) ) {
			$has_sidebars = true;
		}
	}
	...
	$classes = apply_filters( 'singerella_posts_list_class', $classes );
