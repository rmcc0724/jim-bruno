 ...
 * @return string
 */
function singerella_set_post_meta_value( $value ) {
	$queried_obj = apply_filters( 'singerella_queried_object_id', false );
	...
	$curr_opions = 'singerella_' . str_replace( 'theme_mod_', '', current_filter() );
...
function singerella_set_sidebar_position( $value ) {
	$sp = singerella_set_post_meta_value( $value );
...
/** * Sidebar position */
add_filter( 'theme_mod_sidebar_position', 'singerella_set_sidebar_position' );

/** * Header container type */
add_filter( 'theme_mod_header_container_type', 'singerella_set_post_meta_value' );

/** * Content container type */
add_filter( 'theme_mod_content_container_type', 'singerella_set_post_meta_value' );

/** * Footer container type */
add_filter( 'theme_mod_fotter_container_type', 'singerella_set_post_meta_value' );
 ...
 * @return string
 */
function singerella_render_macros( $string ) {

	$macros = apply_filters( 'singerella_data_macros', array(
		'/%%year%%/'           => date( 'Y' ),
		'/%%date%%/'           => date( get_option( 'date_format' ) ),
		'/%%privacy-policy%%/' => singerella_get_privacy_link(),
	) );
 ...
 * @return string
 */
function singerella_get_privacy_link() {
 ...
 * @return string
 */
function singerella_render_icons( $content ) {
	$icons     = singerella_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'singerella_render_icons_callback', $content );
}
 ...
 * @return string
 */
function singerella_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function singerella_get_render_icons_set() {
	return apply_filters( 'singerella_render_icons_set', array(
 ...
 * @return string
 */
function singerella_render_theme_url( $url ) {
 ...
 */
function singerella_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function singerella_post_formats_gallery() {
	$size = singerella_post_thumbnail_size();
	...
	$images = singerella_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
 ...
 * @return bool
 */
function singerella_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, singerella_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function singerella_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'post-thumbnail',
		'fullwidth'    => 'singerella-thumb-l',
		'grid-cols'    => 'singerella-thumb-blog-grid',
		'masonry-cols' => 'singerella-thumb-blog-masonry',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', singerella_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', singerella_theme()->customizer->get_default( 'blog_featured_image' ) );
	...
	if ( in_array(  $layout, array( 'grid-2-cols', 'grid-3-cols' ) ) ) {
		$size       = $args['grid-cols'];
	}
 ...
 * @return array
 */
function singerella_builder_on_class( $post_id ) {
	$builder_on_class = '';
