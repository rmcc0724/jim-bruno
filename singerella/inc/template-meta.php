 ...
 * @return array
 */
function singerella_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! singerella_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'singerella_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'singerella_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'singerella' ) . $network['name'],
